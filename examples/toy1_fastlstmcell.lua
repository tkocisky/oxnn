...
oxnnmc.MaybeLog('toy1_fastlstmcell')
...
if params.version == 1 then
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(nn.SplitTable(2)),
      recurrent = oxnn.ModelUtil.LSTMCell12cl(params.worddim, true),
...
elseif params.version == 2 then
   -- With joined output the output network is many to many.
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(nn.SplitTable(2)),
      recurrent = oxnn.ModelUtil.LSTMCell12cl(params.worddim, true),
      joinedoutput = true,
      output = oxnn.VecsToVecs('nbf', params.worddim, 'table', 'nbf', voc:size(),
               nn.Sequential()
                  :add(nn.Linear(params.worddim, voc:size()))
                  :add(oxnn.LogSoftMaxInplace(true,true))),
      loss = 'nllloss',
   })
elseif params.version == 3 then
   -- With joined output and joined class the output network is many to one and
   -- needs to be reshaped, since the criterion gets only one large batch with all
   -- the outputs in the entire sequence.
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(nn.SplitTable(2)),
      recurrent = oxnn.ModelUtil.LSTMCell12cl(params.worddim, true),
