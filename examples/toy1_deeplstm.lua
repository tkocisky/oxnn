...
if params.version == 0 then
   -- with unoptimized original LSTM cell
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(nn.SplitTable(2)),
      recurrent = oxnn.OneOrTableOfClones(params.layers,
            oxnn.ModelUtil.LSTMCell1(
      ...
      joinedcriterion = false,
      loss = 'nllloss',
      layers = params.layers,
   })

elseif params.version == 1 then
   -- with fast LSTM cell 12cl, and joined output and criterion
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(nn.SplitTable(2)),
      recurrent = oxnn.OneOrTableOfClones(params.layers,
                     oxnn.ModelUtil.LSTMCell12cl(params.worddim, true)),
      joinedoutput = true,
      output =
         nn.Sequential()
             :add(oxnn.VecsToVecs('nbf', params.worddim, 'tensor', 'nbf', voc:size(),
               nn.Sequential()
                  :add(nn.Linear(params.worddim, voc:size()))
                  :add(oxnn.LogSoftMaxInplace(true,true))))
             :add(nn.View(voc:size())),
      joinedcriterion = true,
      loss = 'nllloss',
      layers = params.layers,
   })

elseif params.version == 2 then
   -- with fast LSTM cell 12c with factored input into layer2layer module
   mlp:add(oxnn.SequenceOfWords{
      lookuptable = nn.Sequential():add(nn.LookupTable(voc:size(), params.worddim))
                                   :add(
         oxnn.VecsToVecs('bnf', params.worddim, 'table', 'nbf', 4*params.worddim,
            nn.Linear(params.worddim, 4*params.worddim))),

      recurrent = oxnn.OneOrTableOfClones(params.layers,
                     oxnn.ModelUtil.LSTMCell12c--[[l]](params.worddim, true)),
      ...
      layer2layer = oxnn.TableOfClones(params.layers-1,
         oxnn.VecsToVecs('nbf', params.worddim, 'table', 'nbf', 4*params.worddim,
            nn.Linear(params.worddim, 4*params.worddim))),
   })
end
...
local function CreateInput(ba_it, batch_size, max_doc_len)
   ...
   for i = 1,params.layers-1 do
      table.insert(res[1][1], res[1][1][1])
   end
   if oxnn.cu.use_cuda then
      res[1][2] = res[1][2]:float():cuda()
   end
   return res
end
